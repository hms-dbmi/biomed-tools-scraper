...
const runScrapers = async function ({ COMBINED_CSV_PATH }) {
  try {
    ...
    await Promise.all([
      ...
      scrapeAwesomeToolsVisualization(
        `${DATA_FOLDER}/awesome-tools-visualization_results.csv`,
        params,
      ),
      scrapeAwesomeBiologicalVisualizations(
...
const processCSV = function ({
  ...
  rows.forEach((row) => {
    ...
    const key = columns[PRIMARY_KEY_COLUMN]?.trim()
      .toLowerCase()
      .replace("https://", "")
      .replace("http://", "")
      .replace("www.", ""); // Normalize key field (e.g., External URL)
