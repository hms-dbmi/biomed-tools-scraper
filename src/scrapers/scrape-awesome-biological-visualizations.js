...
const source = "awesome-biological-visualizations";

export const scrapeAwesomeBiologicalVisualizations = async function (
  OUTPUT_FILE,
  { HEADLESS },
) {
  // Launch browser
  ...
  try {
    ...
    const tools = await page.evaluate(() => {
      return Array.from(
        document.querySelectorAll("article ul:not(:first-of-type) li"),
      ).map((tool) => {
        const nameElement = tool.querySelector("a"); // Tool name inside <h3>
        const urlElement = tool.querySelector(
          'a[href^="http"]:not([href*="doi.org"])',
        ); // First valid external URL
