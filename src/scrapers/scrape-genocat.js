...
export const scrapeGenocat = async function (
  OUTPUT_FILE,
  { HEADLESS, DELAY_MS, MAX_CONCURRENT },
) {
  // Launch browser
      ...
      await Promise.all(
        batch.map(async (item) => {
          await new Promise((resolve) =>
            setTimeout(resolve, Math.random() * DELAY_MS),
          ); // Randomized delay
          ...
          try {
            ...
            const externalLink = await toolPage.evaluate(() => {
              const linkElement = document.querySelector(
                'tr td.labelCol + td a[target="_blank"]',
              );
            ...
            fs.appendFileSync(
              OUTPUT_FILE,
              `"${item.name}","${source}","${item.internalUrl}","${externalLink}"\n`,
            );
          } catch (error) {
